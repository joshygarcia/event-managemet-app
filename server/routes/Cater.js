...
const { validateToken, validateTokenManager } = require('../middlewares/AuthMiddleware');
...
router.post('/', validateTokenManager, async (req, res) => {
    try {
        const { name, address, city, state, zip, email, phone, pricePerPerson, availability, description, type, menu, diets, alcohol } = req.body;
